...
def test_default_palindrome_table_exists():
...
def test_run_accepts_palindrome():
    ...
    # универсальная таблица примитивная, может быть и accept, и reject, 
    # но тест проверяет, что машина завершается корректно
    assert m.is_halted()
